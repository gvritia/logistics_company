...
- Обычный push проверенных коммитов в рабочие ветки задач на GitHub разрешён без повторного запроса. Прямые изменения и push в `main` остаются запрещены.
- Pull Request создаёт пользователь; merge также выполняет пользователь. AI-агент не создаёт PR, включая draft, и не выполняет merge, если пользователь отдельно не поручил соответствующее действие.
- Для ближайшего шага КР1 сначала предложить поля модели, статусы и бизнес-правила на рассмотрение пользователя. Реализацию начать после согласования предложения; текущая документация не означает, что модель уже утверждена или реализована.
